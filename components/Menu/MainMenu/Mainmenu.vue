...
const res = computed(() => {
  return menuStore.respons_data_main_menu?.data || [];
});
onMounted(() => {
  ...
  <v-col cols="12">
    <v-row>
      ...
      <v-col cols="12" md="2" class="text-right">
        <v-btn color="primary" @click="goPath('/mainmenu/create')"
          ><v-icon icon="mdi-plus"></v-icon>ເພີ່ມຂໍ້ມູນເມນູ</v-btn
        >
      </v-col>
    </v-row>
    ...
    <v-data-table :items="res" :headers="headers">
      ...
      <template v-slot:item.actions="{ item, index }">
        <v-btn icon="mdi-eye" flat small class="text-primary"></v-btn>
        <v-btn icon="mdi-pen" flat class="text-info"></v-btn>
        <v-btn icon="mdi-delete" flat class="text-error"> </v-btn>
      </template>
    </v-data-table>
  </v-col>
</template>
